...
import { mountColoringGame } from '../games/coloring.js';
...
function render() {
        ...
        <div class="site-container flex flex-col gap-10">
          <section class="flex flex-col gap-6 rounded-3xl bg-white p-6 shadow-soft md:p-10">
            <div class="flex flex-col gap-3">
              <span class="inline-flex w-fit items-center rounded-full bg-accent/10 px-4 py-2 text-xs font-semibold uppercase tracking-wide text-accent">${t('coloringGame.pageEyebrow')}</span>
              <h1 class="text-4xl leading-tight md:text-5xl">${t('coloringGame.pageTitle')}</h1>
              <p class="text-base text-text/80 md:text-lg">${t('coloringGame.pageIntro')}</p>
              <p class="text-sm text-text/70 md:text-base">${t('coloringGame.instructions')}</p>
            </div>
            <div data-coloring-root class="flex flex-col gap-6"></div>
          </section>
        </div>
      </main>
      ${renderFooter()}
  ...
  const gameRoot = appRoot.querySelector('[data-coloring-root]');
  teardown = gameRoot ? mountColoringGame(gameRoot) : null;
}
